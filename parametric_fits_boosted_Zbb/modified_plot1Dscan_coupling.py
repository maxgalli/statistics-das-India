...
#textfit = "#color[3]{%s = %.3f{}^{#plus %.3f}_{#minus %.3f}}" % (fixed_name, val_nom[0], val_nom[1], abs(val_nom[2]))
textfit = "#color[%d]{%s = %.3f{}^{#plus %.3f}_{#minus %.3f}}" % (
    args.main_color,
    fixed_name,
    val_nom[0],
    val_nom[1],
...
pt = ROOT.TPaveText(0.59, 0.82 - 2 * 0.08, 0.95, 0.91, "NDCNB")
pt.AddText(textfit)

for i, other in enumerate(other_scans):
    color = int(other_scans_opts[i][2])  # convert color code to int
    textfit = "#color[%d]{%s = %.3f{}^{#plus %.3f}_{#minus %.3f}}" % (
        color,
        fixed_name,
        other["val"][0],
...
plot.DrawCMSLogo(pads[0], args.logo, args.logo_sub, 11, 0.080, 0.050, 1.2, cmsTextSize=1.0)
